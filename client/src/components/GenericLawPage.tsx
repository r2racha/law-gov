 ...
 * Reuse the established reading rhythm. The only character art is the uploaded
 * P' Racha avatar; the surrounding CSS symbols distinguish each legal context.
 */
...
export function GenericLawPage({ config }: { config: GenericLawConfig }) {
  ...
  const filtered = useMemo(() => config.articles.filter((article) => {
    const inSection = activeSection === "ทั้งหมด" || article.section === activeSection;
    const matchesQuery = !normalized || [article.number, article.section, article.keyPoint, article.plainSummary, article.lawText].join(" ").toLowerCase().includes(normalized);
    return inSection && matchesQuery;
  }), [activeSection, config.articles, normalized]);
  ...
  const navItems = [["ภาพรวม", "overview"], ["อ่านรายข้อ", "articles"], ["เล่าทั้งฉบับ", "whole-story"], ["ข้อสอบทบทวน", "quiz"]] as const;
  ...
  return <div style={style} className="law-generic min-h-screen overflow-x-hidden bg-[#FFFDF8] text-[#34322D]">
    <header className="sticky top-0 z-50 border-b border-[#173C3A]/10 bg-[#FFFDF8]/92 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-3 lg:px-10">
        <button onClick={() => scrollTo("top")} className="flex items-center gap-3 text-left"><img src={logoUrl} alt="สัญลักษณ์กฎหมายฉบับเล่าเรื่อง" className="h-16 w-16 object-contain drop-shadow-sm" /><span className="leading-tight"><strong className="block text-[20px] font-semibold text-[#0E766F]">กฎหมายฉบับเล่าเรื่อง</strong><span className="hidden text-xs text-[#6C6A63] sm:block">{config.headerLabel}</span></span></button>
        ...
        <button onClick={() => setMenu((value) => !value)} className="grid h-10 w-10 place-items-center rounded-full bg-[var(--law-soft)] text-[var(--law-primary-dark)] md:hidden">{menu ? <X size={20} /> : <Menu size={20} />}</button>
      </div>
    ...
    <main id="top">
      ...
      <section id="articles" className="scroll-mt-24 bg-[var(--law-soft)] px-5 py-20 lg:px-10"><div className="mx-auto max-w-[1280px]"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="section-kicker">ห้องอ่านราย{config.articleNoun}</p><h2 className="mt-3 text-4xl font-semibold leading-tight text-[var(--law-primary-dark)]">เลือกอ่านแบบที่<br />ไม่หลงในคำยาก</h2></div><div className="max-w-md rounded-2xl border border-[var(--law-primary)]/15 bg-white p-4 text-sm leading-6 text-[#5B6662]"><Lightbulb className="mb-1 inline h-4 w-4 text-[#B77A37]" /> เริ่มจากกล่อง “เล่าง่ายๆสไลต์พี่ราช” แล้วเปิดตัวบทจริงเมื่ออยากตรวจถ้อยคำ</div></div><div className="mt-10 grid gap-5 lg:grid-cols-[260px_1fr]">
        <aside className="lg:sticky lg:top-24 lg:h-fit"><div className="border border-[var(--law-primary)]/15 bg-[#FFFDF8] p-4 shadow-sm"><p className="mb-3 text-sm font-semibold text-[var(--law-primary-dark)]">เลือกหมวด</p><div className="flex gap-2 overflow-x-auto pb-2 lg:flex-col lg:overflow-visible">{["ทั้งหมด", ...sections].map((section) => <button key={section} onClick={() => setActiveSection(section)} className={`border-l-4 px-3 py-2 text-left text-sm font-medium transition lg:whitespace-normal ${activeSection === section ? "border-[var(--law-primary)] bg-white text-[var(--law-primary-dark)]" : "border-transparent text-[#666158] hover:bg-[#F6F0E8]"}`}>{section}</button>)}</div></div><div className="mt-5 border-l-4 border-[#D4A95F] bg-[#FFF6E6] p-4 text-sm leading-6 text-[#785E32]"><strong className="block text-[#9C7134]">วิธีเดินบทเรียน</strong>เลือกหมวด → เปิดคำอธิบาย → ค่อยตรวจตัวบทจริง</div></aside>
        <div><label className="flex items-center gap-3 border border-[var(--law-primary)]/15 bg-white px-4 py-3 shadow-sm"><Search size={19} className="text-[var(--law-primary)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm outline-none placeholder:text-[#A39F95]" placeholder={`ค้นหา${config.articleNoun} คำสำคัญ หรือคำอธิบาย…`} /></label><p className="mt-4 text-sm text-[#66736F]">พบ <strong className="text-[var(--law-primary-dark)]">{filtered.length}</strong> {config.articleNoun}</p><div className="mt-6 space-y-12">{groups.map((group, chapterIndex) => <section key={group.section} className="relative scroll-mt-28"><div className="generic-chapter-spread"><div className="relative z-10 max-w-xl"><p className="chapter-scene">ส่วนที่ {String(chapterIndex + 1).padStart(2, "0")} · {group.section}</p><h3>{group.section}</h3><p className="chapter-narrator"><span>พี่ราชบอก</span>อ่านตัวบทในส่วนนี้เพื่อดูเงื่อนไข ผู้มีหน้าที่ และขั้นตอนที่เกี่ยวข้อง</p></div><img src={rachaGuideUrl} alt={`พี่ราชแนะนำ ${group.section}`} className="chapter-mascot" /><span className="chapter-count">{group.articles.length}<small>{config.articleNoun}</small></span></div><Accordion type="multiple" className="article-route mt-5 space-y-3">{group.articles.map((article, index) => <Fragment key={article.number}><ArticleCard article={article} articleNoun={config.articleNoun} show={!!showLawText[article.number]} onToggle={() => setShowLawText((current) => ({ ...current, [article.number]: !current[article.number] }))} />{index < group.articles.length - 1 && (index + 1) % 8 === 0 && <aside className="generic-interlude"><div><span>จุดพักบทเรียน · พี่ราชชวนทวน</span><p>ทบทวนข้อความในตัวบทส่วนนี้ก่อนเปิดอ่านข้อต่อไป</p></div><img src={rachaGuideUrl} alt="พี่ราชชวนทวนบทเรียน" /></aside>}</Fragment>)}</Accordion></section>)}</div>{!filtered.length && <div className="mt-4 border border-dashed border-[var(--law-primary)]/30 bg-white px-6 py-12 text-center text-[#6B7068]"><CircleHelp className="mx-auto mb-3 text-[var(--law-primary)]" />ไม่พบ{config.articleNoun}ที่ตรงกับคำค้น ลองใช้คำสั้นลงหรือเลือก “ทั้งหมด”</div>}{filtered.length > 0 && <aside className="mt-14 overflow-hidden border border-[var(--law-primary)]/15 bg-white shadow-[0_14px_30px_rgba(57,82,48,.08)]"><div className="grid items-center gap-4 px-6 py-7 sm:grid-cols-[1fr_auto] sm:px-10"><div><p className="section-kicker">เดินทางถึงปลายบท</p><h3 className="mt-2 text-2xl font-semibold text-[var(--law-primary-dark)]">อ่าน{config.articleNoun}สุดท้ายแล้ว<br />พี่ราชดีใจด้วยครับ</h3><p className="mt-3 max-w-xl text-sm leading-6 text-[#5B6662]">พร้อมแล้ว ลองไปทบทวนด้วยแบบทดสอบ 20 ข้อ เพื่อเช็กความเข้าใจของตัวเอง</p></div><img src={rachaCelebrateUrl} alt="พี่ราชดีใจเมื่ออ่านตัวบทครบแล้ว" className="mx-auto h-44 w-auto object-contain sm:h-52" /></div></aside>}</div>
      </div></div></section>

      <section id="quiz" className="scroll-mt-24 bg-[#FFF8EA] px-5 py-20 lg:px-10"><div className="mx-auto max-w-[1180px]"><div><p className="section-kicker">ข้อสอบทบทวน</p><h2 className="mt-3 text-4xl font-semibold leading-tight text-[#4D4A32]">ลองตอบก่อน<br />แล้วค่อยดูเหตุผล</h2><p className="mt-5 text-lg leading-8 text-[#6C6650]">แบบทดสอบ 20 ข้อ</p><div className="mt-7 max-w-2xl border border-[#E6D5A9] bg-[#FFFDF8] p-5"><div className="flex justify-between text-sm font-semibold text-[#806E40]"><span>ความคืบหน้า</span><span>{answered} / {config.quiz.length} ข้อ</span></div><Progress value={(answered / config.quiz.length) * 100} className="mt-3 h-3 bg-[#F2E7CA] [&>div]:bg-[var(--law-primary)]" />{answered === config.quiz.length && <p className="mt-4 text-sm leading-6 text-[#566048]">ทำครบแล้ว: ตอบถูก <strong className="text-[var(--law-primary-dark)]">{correct}</strong> จาก {config.quiz.length} ข้อ <button onClick={() => setAnswers({})} className="ml-2 font-bold text-[var(--law-primary-dark)] underline">เริ่มใหม่</button></p>}</div></div><div className="mt-12 grid gap-5 md:grid-cols-2">{config.quiz.map((question) => <QuizCard key={question.id} question={question} selected={answers[question.id]} onAnswer={(choice) => setAnswers((current) => ({ ...current, [question.id]: choice }))} />)}</div></div></section>
    </main>
...
function ArticleCard({ article, articleNoun, show, onToggle }: { article: LawArticle; articleNoun: string; show: boolean; onToggle: () => void }) {
  return <AccordionItem value={`article-${article.number}`} className="lesson-card border border-[var(--law-primary)]/12 bg-[#FFFDF8] px-0 shadow-[0_8px_18px_rgba(53,83,45,.04)]"><AccordionTrigger className="items-start gap-5 px-5 py-5 text-left hover:no-underline"><span className="article-ribbon generic-ribbon"><em>{articleNoun}</em><b>{article.number}</b></span><span className="min-w-0 flex-1"><span className="mb-2 inline-flex rounded-full bg-[var(--law-soft)] px-2.5 py-1 text-[11px] font-bold text-[var(--law-primary-dark)]">{article.section}</span><span className="block text-base font-semibold leading-7 text-[#3B5233]">{article.keyPoint}</span></span></AccordionTrigger><AccordionContent className="px-5 pb-6"><div className="grid gap-5 border-t border-[var(--law-primary)]/10 pt-6 lg:grid-cols-[1.6fr_.9fr]"><div className="generic-postit"><p className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--law-primary-dark)]"><Sparkles size={16} /> เล่าง่ายๆสไลต์พี่ราช</p><p className="text-[15px] leading-8 text-[#4D504B]">{article.plainSummary}</p></div><div className="generic-memory"><p><strong>สาระของ{articleNoun}</strong>{article.keyPoint}</p></div></div><div className="mt-5"><button onClick={onToggle} className="inline-flex items-center gap-2 border-b border-[var(--law-primary)]/40 pb-1 text-sm font-bold text-[var(--law-primary-dark)]"><FileText size={16} /> {show ? "ซ่อนตัวบทจริง" : "เปิดตัวบทจริง"}</button>{show && <div className="mt-4 max-h-[460px] overflow-y-auto border border-[var(--law-primary)]/15 bg-[#FBFCF5] p-5 font-serif text-[15px] leading-8 text-[#494A41]">{article.lawText.split("\n").filter(Boolean).map((line, index) => <p key={`${article.number}-${index}`}>{line}</p>)}</div>}</div></AccordionContent></AccordionItem>;
}

 ...
 * An asymmetric reading journey with P' Niti, Civic Teal wayfinding, paper-like layers,
 * and one helpful learning action per view. Text must always remain easy to read.
 */
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import { BookOpen, ChevronRight, CircleHelp, FileText, GraduationCap, Lightbulb, Menu, Search, Sparkles, X } from "lucide-react";
...
const mascotUrl = "https://files.manuscdn.com/user_upload_by_module/session_file/310519663866287321/ghamaNOVaeeexEMb.png";
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#FFFDF8] text-[#34322D]">
      <header className="sticky top-0 z-50 border-b border-[#173C3A]/10 bg-[#FFFDF8]/92 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-3 lg:px-10">
          <button onClick={() => scrollTo("top")} className="flex items-center gap-3 text-left" aria-label="กลับไปด้านบน">
            <img src={logoUrl} alt="สัญลักษณ์กฎหมายฉบับเล่าเรื่อง" className="h-11 w-11 object-contain" />
      ...
      <main id="top">
        <section className="relative isolate overflow-hidden bg-[#E9F5F1] pb-14 pt-10 lg:pb-20 lg:pt-16">
          ...
          <div className="mx-auto grid max-w-[1440px] items-center gap-7 px-5 lg:grid-cols-[.92fr_1.08fr] lg:px-10">
            <div className="relative z-10 pb-4 lg:py-10">
              ...
              <p className="mt-6 max-w-xl text-lg leading-8 text-[#595750]">จากเอกสารกฎหมายสู่เส้นทางเรียนรู้ที่อ่านง่ายขึ้น มีคำแปลภาษาชาวบ้าน ตัวบทอ้างอิง และสนามสอบทบทวนท้ายบท</p>
            ...
            <div className="hero-stage relative min-h-[315px] overflow-hidden rounded-[38%_62%_44%_56%/45%_38%_62%_55%] border-[10px] border-[#FFFDF8]/80 bg-[#D7ECE6] shadow-[0_30px_60px_rgba(26,75,71,.14)] lg:min-h-[510px]">
              <span className="hero-paper hero-paper-one" /><span className="hero-paper hero-paper-two" /><span className="hero-star">✦</span><img src={mascotUrl} alt="พี่นิติพาเดินผ่านเส้นทางการเรียนรู้กฎหมาย" className="absolute bottom-0 right-[5%] z-10 h-[93%] w-[82%] object-contain object-bottom" />
              <div className="absolute bottom-6 left-6 max-w-[215px] rounded-2xl bg-[#FFFDF8]/92 px-4 py-3 shadow-lg backdrop-blur"><p className="text-sm font-semibold text-[#0E625E]">พี่นิติชวนอ่าน</p><p className="mt-1 text-xs leading-5 text-[#5F5A52]">เปิดคำแปลก่อน แล้วกดดูตัวบทจริงได้ทุกมาตรา</p></div>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="mt-6 space-y-12">
                  {groupedArticles.map((group, chapterIndex) => {
                    ...
                    return <section key={group.section} className="relative scroll-mt-28" id={`chapter-${chapterIndex + 1}`}>
                      <div className={`chapter-spread ${meta.tone}`}><div className="relative z-10 max-w-xl"><p className="chapter-scene">บทที่ {String(chapterIndex + 1).padStart(2, "0")} · {meta.scene}</p><h3>{meta.title}</h3><p className="chapter-narrator"><span>พี่นิติบอก</span>{meta.narrator}</p></div><img src={mascotUrl} alt={`พี่นิติแนะนำ ${meta.title}`} className="chapter-mascot" /><span className="chapter-count">{group.articles.length}<small>มาตรา</small></span></div>
                      <Accordion type="multiple" className="mt-5 space-y-3">
                        {group.articles.map((article) => {
                          const repealed = article.lawText.includes("(ยกเลิก)");
                          return <AccordionItem key={article.number} value={`article-${article.number}`} className="lesson-card border border-[#0E8F8A]/12 bg-[#FFFDF8] px-0 shadow-[0_8px_18px_rgba(38,78,72,.04)]">
                            <AccordionTrigger className="items-start gap-5 px-5 py-5 text-left hover:no-underline"><span className="article-ribbon">มาตรา <b>{article.number}</b></span><span className="min-w-0 flex-1"><span className="mb-2 flex flex-wrap gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${sectionColors[article.section] ?? "bg-[#eee]"}`}>{article.section}</span>{repealed && <span className="rounded-full bg-[#F0ECE5] px-2.5 py-1 text-[11px] font-bold text-[#70685E]">ถูกยกเลิก</span>}</span><span className="block text-base font-semibold leading-7 text-[#234743]">{article.keyPoint}</span></span></AccordionTrigger>
                            <AccordionContent className="px-5 pb-6"><div className="grid gap-5 border-t border-[#0E8F8A]/10 pt-6 lg:grid-cols-[1.6fr_.9fr]"><div className="postit-note"><p className="mb-2 flex items-center gap-2 text-sm font-bold text-[#0E766F]"><Sparkles size={16} /> แปลแบบพี่นิติ</p><p className="text-[15px] leading-8 text-[#4D504B]">{article.plainSummary}</p></div><div className="memory-cue"><img src={mascotUrl} alt="" className="h-16 w-auto self-end" /><p><strong>จำไว้</strong>{article.keyPoint}</p></div></div><div className="mt-5"><button onClick={() => setShowLawText((current) => ({ ...current, [article.number]: !current[article.number] }))} className="inline-flex items-center gap-2 border-b border-[#0E8F8A]/40 pb-1 text-sm font-bold text-[#0E766F] hover:text-[#075F5A]"><FileText size={16} /> {showLawText[article.number] ? "ซ่อนตัวบทจริง" : "เปิดตัวบทจริงจากไฟล์ต้นฉบับ"}</button>{showLawText[article.number] && <div className="mt-4 max-h-[460px] overflow-y-auto border border-[#DBCDBE] bg-[#FBF7F0] p-5 font-serif text-[15px] leading-8 text-[#49443C]">{article.lawText.split("\n").filter(Boolean).map((line, index) => <p key={`${article.number}-${index}`} className={line.trim().startsWith("(") ? "pl-4" : ""}>{line}</p>)}</div>}</div></AccordionContent>
                          </AccordionItem>;
                        })}
                      </Accordion>
                    </section>;
        ...
        <section id="quiz" className="scroll-mt-24 bg-[#FFF7EB] px-5 py-20 lg:px-10">
          <div className="mx-auto max-w-[1180px]">
            <div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center"><div><p className="section-kicker">สนามสอบทบทวน</p><h2 className="mt-3 text-4xl font-semibold leading-tight text-[#4D3E2C]">ลองตอบก่อน<br />แล้วค่อยดูเหตุผล</h2><p className="mt-5 max-w-lg text-lg leading-8 text-[#6C5E50]">แบบฝึก 20 ข้อคัดจากสาระสำคัญในตัวบท ตอบแล้วเห็นเฉลยทันทีเพื่อพากลับไปทบทวนมาตราที่เกี่ยวข้อง</p><div className="mt-7 border border-[#EAC89C] bg-[#FFFDF8] p-5"><div className="flex justify-between text-sm font-semibold text-[#866040]"><span>ความคืบหน้า</span><span>{answered} / {quizQuestions.length} ข้อ</span></div><Progress value={quizProgress} className="mt-3 h-3 bg-[#F2E1C7] [&>div]:bg-[#E17A38]" />{answered === quizQuestions.length && <p className="mt-4 text-sm leading-6 text-[#5B604C]">ทำครบแล้ว: ตอบถูก <strong className="text-[#466E45]">{correct}</strong> จาก {quizQuestions.length} ข้อ <button onClick={() => setAnswers({})} className="ml-2 font-bold text-[#0E766F] underline">เริ่มใหม่</button></p>}</div></div><div className="quiz-canvas relative min-h-[300px] overflow-hidden rounded-[65px_26px_65px_26px] border-[10px] border-white shadow-[0_20px_40px_rgba(90,60,35,.12)]"><span className="quiz-check">✓</span><span className="quiz-confetti quiz-confetti-one" /><span className="quiz-confetti quiz-confetti-two" /><span className="quiz-confetti quiz-confetti-three" /><img src={mascotUrl} alt="พี่นิติชวนเฉลยแบบฝึกหัด" className="absolute bottom-0 left-1/2 z-10 h-[93%] w-[70%] -translate-x-1/2 object-contain object-bottom" /></div></div>
            <div className="mt-12 grid gap-5 md:grid-cols-2">
              {quizQuestions.map((question) => { const selected = answers[question.id]; const hasAnswered = selected !== undefined; return <article key={question.id} className="border border-[#E6CFAE] bg-[#FFFDF8] p-5 shadow-[0_10px_22px_rgba(89,62,37,.06)]"><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-[#FBE4C9] px-3 py-1 text-xs font-bold text-[#A05B26]">ข้อ {question.id}</span><span className="text-xs font-semibold text-[#0E766F]">{question.articleRef}</span></div><h3 className="mt-4 text-base font-semibold leading-7 text-[#4E4436]">{question.question}</h3><div className="mt-4 space-y-2">{question.choices.map((choice, index) => { const isCorrect = index === question.answerIndex; const isSelected = selected === index; const state = hasAnswered ? (isCorrect ? "border-[#70A66E] bg-[#EFF8EB] text-[#37693B]" : isSelected ? "border-[#E49B8C] bg-[#FFF0ED] text-[#AA4C3C]" : "border-[#EADFD1] text-[#7D7469]") : "border-[#EADFD1] text-[#665F56] hover:border-[#0E8F8A] hover:bg-[#F0F8F6]"; return <button key={choice} disabled={hasAnswered} onClick={() => setAnswers((current) => ({ ...current, [question.id]: index }))} className={`flex w-full items-start gap-3 border px-3 py-2.5 text-left text-sm leading-6 transition disabled:cursor-default ${state}`}><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full border border-current text-[10px] font-bold">{String.fromCharCode(65 + index)}</span>{choice}</button>; })}</div>{hasAnswered && <div className={`mt-4 border-l-4 px-4 py-3 text-sm leading-6 ${selected === question.answerIndex ? "border-[#6E9D69] bg-[#F1F8ED] text-[#406744]" : "border-[#E17A38] bg-[#FFF4E8] text-[#865735]"}`}><strong>{selected === question.answerIndex ? "ถูกต้อง" : "เฉลย"}</strong><p>{question.explanation}</p></div>}</article>; })}
            </div>
          </div>
        </section>

        <section className="bg-[#174945] px-5 py-16 text-white lg:px-10"><div className="mx-auto grid max-w-[1180px] gap-8 md:grid-cols-[auto_1fr_auto] md:items-center"><img src={mascotUrl} alt="พี่นิติ" className="mx-auto h-32 w-auto object-contain md:h-40" /><div><p className="text-sm font-semibold tracking-[.12em] text-[#9FD8D1]">ก่อนปิดหนังสือ</p><h2 className="mt-2 text-3xl font-semibold leading-tight">กฎหมายอ่านยากได้<br />แต่การเริ่มเข้าใจไม่จำเป็นต้องยาก</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#D7EFEB]">เว็บไซต์นี้เรียบเรียงจากไฟล์ที่จัดส่งเพื่อการเรียนรู้ คำอธิบายภาษาชาวบ้านช่วยให้เริ่มต้นอ่าน แต่ไม่ใช่คำปรึกษากฎหมายเฉพาะกรณี</p></div><Button onClick={() => scrollTo("top")} variant="outline" className="rounded-full border-white/30 bg-white/10 text-white hover:bg-white/20 hover:text-white">กลับไปเริ่มต้น</Button></div></section>
      </main>
      <footer className="bg-[#123B38] px-5 py-5 text-center text-xs leading-5 text-[#BFE1DC]">แหล่งตัวบท: ไฟล์ “1.2.1 วิธีปฏิบัติราชการทางปกครอง.md” ที่ผู้ใช้จัดส่ง · ควรตรวจสอบฉบับแก้ไขเพิ่มเติมล่าสุดก่อนใช้อ้างอิงทางกฎหมาย</footer>
    </div>
  );
}
